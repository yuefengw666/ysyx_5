...
extern "C" void set_npc_state(int state, const svOpenArrayHandle pc, int halt_ret);

VL_INLINE_OPT void Vysyx_22040237_rv_single_cyc_cpu_top___024unit____Vdpiimwrap_set_npc_state__Vdpioc2_TOP____024unit(IData/*31:0*/ state, const QData/*63:0*/ &pc, IData/*31:0*/ halt_ret) {
    VL_DEBUG_IF(VL_DBG_MSGF("+        Vysyx_22040237_rv_single_cyc_cpu_top___024unit____Vdpiimwrap_set_npc_state__Vdpioc2_TOP____024unit\n"); );
    // Body
    int state__Vcvt;
    for (size_t state__Vidx = 0; state__Vidx < 1; ++state__Vidx) state__Vcvt = state;
    static const VerilatedVarProps pc__Vopenprops(VLVT_UINT64, VLVD_IN, VerilatedVarProps::Packed(), 63, 0);
    VerilatedDpiOpenVar pc__Vopenarray (&pc__Vopenprops, &pc);
    int halt_ret__Vcvt;
    for (size_t halt_ret__Vidx = 0; halt_ret__Vidx < 1; ++halt_ret__Vidx) halt_ret__Vcvt = halt_ret;
    set_npc_state(state__Vcvt, &pc__Vopenarray, halt_ret__Vcvt);
}
