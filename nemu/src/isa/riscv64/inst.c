...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT(BITS(src1 << BITS(src2,4,0),31,0),31) );
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2 );
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = ((unsigned)src1 < (unsigned)src2) ? 1 : 0 );
  ...
  //movsx
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT(BITS((unsigned)src1 << BITS(src2,5,0),31,0),32) );
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT(((signed)BITS(src1,31,0)) >> BITS(src2,5,0),32) );
  //mul-longlong
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", or     , R, R(dest) = src1 * src2 );
  //recursion
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = src1 );
  //shift
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT(BITS((unsigned)src1 >> BITS(src2,5,0),31,0),32) );
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, if(BITS(src2,30,30)==1) R(dest) = SEXT(((signed)BITS(src1,31,0) >> BITS(src2,4,0)),32) );
